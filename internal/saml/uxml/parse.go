...
import (
	...
	"github.com/sebdroid/ssosebby/internal/saml/uxml/stack"
)

var parser = participle.MustBuild[doc](
	participle.Lexer(lexer.MustStateful(lexer.Rules{
		"Root": {
			{Name: "<?", Pattern: `<\?`, Action: lexer.Push("Declaration")},
			{Name: "<", Pattern: `<`, Action: lexer.Push("Element")},
			{Name: "BeginText", Pattern: `[^<]`, Action: lexer.Push("Text")},
		},
		"Declaration": {
			{Name: "?>", Pattern: `\?>`, Action: lexer.Pop()},
			{Name: "=", Pattern: `=`, Action: nil},
			{Name: "S", Pattern: `[ \t\r\n]+`, Action: nil},
			{Name: "Name", Pattern: `[a-zA-Z][a-zA-Z0-9:]*`, Action: nil},
