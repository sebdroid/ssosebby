...
const authCountSCIMUsersByActive = `-- name: AuthCountSCIMUsersByActive :one
select count(*)
from scim_users
where scim_directory_id = $1
  and deleted = false
  and (attributes->>'active')::boolean = $2::boolean
`

type AuthCountSCIMUsersByActiveParams struct {
	ScimDirectoryID uuid.UUID
	Active          bool
}

func (q *Queries) AuthCountSCIMUsersByActive(ctx context.Context, arg AuthCountSCIMUsersByActiveParams) (int64, error) {
	row := q.db.QueryRow(ctx, authCountSCIMUsersByActive, arg.ScimDirectoryID, arg.Active)
	var count int64
	err := row.Scan(&count)
	return count, err
}
...
const authGetSCIMUserByEmailAndActive = `-- name: AuthGetSCIMUserByEmailAndActive :one
select id, scim_directory_id, email, deleted, attributes
from scim_users
where scim_directory_id = $1
  and email = $2
  and deleted = false
  and (attributes->>'active')::boolean = $3::boolean
`

type AuthGetSCIMUserByEmailAndActiveParams struct {
	ScimDirectoryID uuid.UUID
	Email           string
	Active          bool
}

func (q *Queries) AuthGetSCIMUserByEmailAndActive(ctx context.Context, arg AuthGetSCIMUserByEmailAndActiveParams) (ScimUser, error) {
	row := q.db.QueryRow(ctx, authGetSCIMUserByEmailAndActive, arg.ScimDirectoryID, arg.Email, arg.Active)
	var i ScimUser
	err := row.Scan(
		&i.ID,
		&i.ScimDirectoryID,
...
const authListSCIMUsersByActive = `-- name: AuthListSCIMUsersByActive :many
select id, scim_directory_id, email, deleted, attributes
from scim_users
where scim_directory_id = $1
  and deleted = false
  and (attributes->>'active')::boolean = $4::boolean
order by id
offset $2 limit $3
`

type AuthListSCIMUsersByActiveParams struct {
	ScimDirectoryID uuid.UUID
	Offset          int32
	Limit           int32
	Active          bool
...
func (q *Queries) AuthListSCIMUsersByActive(ctx context.Context, arg AuthListSCIMUsersByActiveParams) ([]ScimUser, error) {
	rows, err := q.db.Query(ctx, authListSCIMUsersByActive,
		arg.ScimDirectoryID,
		arg.Offset,
		arg.Limit,
		arg.Active,
